...
```sh
pip install pydantic-ai pydantic-ai-skills
```
...
# Output
# 当前可用的技能如下:
# 1. ossbx: 获取当前操作系统的内存使用率、硬盘占用、本地及公网 IP 地址以及 CPU/系统温度。
```

<details>
<summary>PydanticAI 加载本地目录 Skill 与远程 Git 仓库 Skill，点击展开</summary>
...
registry = GitSkillsRegistry("https://github.com/drawmoon/llms_skills.git")
...
# 当前可用的技能如下:
# 1. ossbx: 获取当前操作系统的内存使用率、硬盘占用、本地及公网 IP 地址以及 CPU/系统温度。
# 2. llmwiki: A pattern for building personal knowledge bases using LLMs.
```

</details>

<details>
<summary>PydanticAI 声明式 Spec 驱动，点击展开</summary>
...
from pydantic_ai_skills import GitSkillsRegistry, SkillsCapability, SkillsToolset
from pydantic_ai_skills.registries import FilteredRegistry

os.environ["OPENAI_BASE_URL"] = "http://localhost:4000"
os.environ["OPENAI_API_KEY"] = "sk-1234"

SKILL_BASE_PATH = "./skills"


@dataclass
class SkillsRegistry(AbstractCapability[Any]):
    uri: str
    ...
    def _get_cap(self):
        ...
        if self.uri.endswith(".git"):
            registry = GitSkillsRegistry(self.uri)
            if allowed_skills:
                registry = FilteredRegistry(
                    registry, predicate=lambda s: s.name in allowed_skills
                )
            return SkillsCapability(registries=[registry], auto_reload=True)

        skill_dirs = (
            [f"{SKILL_BASE_PATH}/{s}" for s in allowed_skills]
            if allowed_skills
            else [SKILL_BASE_PATH]
        )
        return SkillsCapability(directories=skill_dirs, auto_reload=True)

    def get_toolset(self) -> SkillsToolset | None:
        return self._get_cap().get_toolset()
...
agent = Agent.from_spec(
    {
        "model": "qwen3.5:9b",
        "capabilities": [
            {"SkillsRegistry": {"uri": "skill://default", "skills": ["ossbx"]}},
            {
                "SkillsRegistry": {
                    "uri": "https://github.com/drawmoon/llms_skills.git",
                    "skills": ["llmwiki"],
                },
            },
        ],
    },
    custom_capability_types=[SkillsRegistry],
)
